...
class TopBarVisualizerView @JvmOverloads constructor(
    ...
    private var visualizationMode = TopBarVisualizationMode.ANALOG_METER
    ...
    fun applySettings(
        ...
        visualizationMode = mode
        this.limitFpsTo30 = limitFpsTo30
        this.visualizationDelayMs = visualizationDelayMs.coerceIn(-250, 250)
    ...
    private fun fillDelayedSnapshot(
        ...
        val delayed = resolveSnapshotForOffset(history, latest, nowMs)
        target.fill(0f)
        val source = delayed ?: latest
        System.arraycopy(source, 0, target, 0, min(target.size, source.size))
    }

    private fun resolveSnapshotForOffset(
        history: ArrayDeque<TimedSnapshot>,
        latest: FloatArray,
        nowMs: Long
    ): FloatArray? {
        ...
        return when {
            visualizationDelayMs > 0 -> resolveDelayedSnapshot(history, nowMs)?.values
            visualizationDelayMs < 0 -> resolveLeadSnapshot(history, latest)
            else -> history.lastOrNull()?.values ?: latest
        }
    }
    ...
    private fun resolveLeadSnapshot(
        history: ArrayDeque<TimedSnapshot>,
        latest: FloatArray
    ): FloatArray {
        val latestSnapshot = history.lastOrNull() ?: return latest
        val previousSnapshot = history.elementAtOrNull(history.size - 2) ?: return latestSnapshot.values
        val deltaMs = (latestSnapshot.timestampMs - previousSnapshot.timestampMs).coerceAtLeast(1L)
        val leadRatio = (-visualizationDelayMs).toFloat() / deltaMs.toFloat()
        val clampedRatio = leadRatio.coerceIn(0f, 3f)
        val predicted = FloatArray(latest.size)
        for (index in predicted.indices) {
            val current = latestSnapshot.values.getOrElse(index) { latest.getOrElse(index) { 0f } }
            val previous = previousSnapshot.values.getOrElse(index) { current }
            predicted[index] = current + (current - previous) * clampedRatio
        }
        return predicted
    }

    private fun pruneSnapshotHistory(history: ArrayDeque<TimedSnapshot>, nowMs: Long) {
        val retentionMs = kotlin.math.abs(visualizationDelayMs) + 400L
